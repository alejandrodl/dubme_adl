import os
import time
from utils import clear_make_directory, extract_audio_from_video, downsample_and_mono_from_path, denoise_audio_file, get_noise_files, transcribe_audio_file, compute_metrics, save_metrics
...
# STEP 5 - Transcribe each audio file and denoised audio file and save it to transcriptions_directory
for audio_path in audio_paths:
...
for denoised_audio_path in denoised_audio_paths:
    ...
    metrics_dict = compute_metrics(metrics_dict, denoised_audio_path, original_audio_path, abs_metrics=[], rel_metrics=['stoi', 'sisdr'])
